...
const AddOffice = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
        const response = await axios.post('http://localhost:5000/offices', newOffice);
  ...
  return (
    <form onSubmit={handleSubmit} className='p-4 border rounded shadow-md mb-4'>
        ...
        <div className='mb-4'>
            ...
            <select value={roomId} onChange={(e) => setRoomId(e.target.value)} className='w-full p-2 border rounded' required>
        ...
        <button type='submit' className='bg-blue-500 text-white p-2 rounded'>Ajouter le bureau</button>
    </form>
  );
};
